...
# Settings
SETTINGS: dict = {"ledCount": int, "rgb": tuple, "effect": int}
...
def parse_xlume_data(data: str) -> dict:
    parsed_data: dict = {"ledCount": None, "rgb": None, "effect": None}
    ...
    while True:
        if SENSE_PIN.value() == 1:
            ...
            if SMC_UART.any():
                await sleep_ms(
                    5
                )  # Wait for final few bits to arrive (somehow fixes a cutoff bug)
                data = SMC_UART.read()
                if data is not None:
